...
function HeroCarousel({ onSelect }) {
    // Google Drive 공유링크 -> 직접 보기 링크로 변환
    ...
    const normalizeSrc = (src) => {
        if (!src) return null;
        if (/^https?:\/\//i.test(src)) return driveToDirect(src);     // 웹/드라이브
        if (src.startsWith("/")) return src;                           // 절대 경로 (public 루트)
        // 상대 경로면 public 기준으로 강제
    ...
    const start = () => {
        ...
        timerRef.current = setInterval(() => setIndex((p) => (p + 1) % slides.length), 5500);
    };
    const stop = () => {
        if (!timerRef.current) return;
        ...
        <div className="hero-carousel" onMouseEnter={stop} onMouseLeave={start}>
            {slides.map((slideData, i) => (
                <div
                    ...
                    style={{ background: slideData.bg }}
                >
                    <div className="hero-content">
                        <div className="hero-eyebrow">{slideData.eyebrow}</div>
                        <h2
                        ...
                        <p className="hero-subtitle">{slideData.subtitle}</p>
                    </div>
            ...
            <button aria-label="이전" onClick={prev} className="hero-arrow prev">‹</button>
            <button aria-label="다음" onClick={next} className="hero-arrow next">›</button>
                ...
                <div id="mainPage" className="theme-ink">
                    ...
                    <div className="mac-container">
                        <div className="mac-content">
                                        ...
                                        <div className="d-flex justify-content-between align-items-start">
                                            <div>
                                                <div className="d-flex align-items-center gap-2">
                                                    ...
                                                    <h2 className="mb-1">구글 드라이브</h2>
                                                </div>
                                                <p className="mb-0" style={{ opacity: 0.8 }}>
                                                    포트폴리오 자료를 업로드·정리하고, 시트와 연동해 PPT를 빠르게 만드세요.
                                                </p>
                                            </div>
                                            ...
                                            <div className="d-flex align-items-center gap-2">
                                                <label
                                                    ...
                                                    className={`btn ${isUploadLoading ? 'btn-outline-secondary disabled file-upload-btn' : 'btn-outline-success'} btn-sm`}
                                                >
                                                    {isUploadLoading ? (
                                                        <>
                                                            <span className="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span>
                            ...
                            <span className="badge rounded-pill bg-dark-subtle border border-secondary-subtle" style={{ opacity: .9 }}>
                              {driveViewMode === 'all' ? '전체 파일' : '포트폴리오 폴더'}
                            </span>
                                            {currentPath.map((node, i) => (
                                                <span key={node.id} className="badge rounded-pill bg-dark-subtle border border-secondary-subtle" style={{ opacity: .9 }}>
                                {node.name}
                              </span>
                                            ))}
                                        </div>
                                                ...
                                                <div className="empty-state py-5">
                                                    {isLoggedIn || isInitializing ? (
                                                        <>
                                                            ...
                                                            <p className="mt-3">구글 드라이브 연동 중…</p>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <i className="fas fa-exclamation-circle fa-2x mb-2"></i>
                                                            <p className="mb-1">구글 드라이브가 연동되지 않았습니다.</p>
                                                            <small className="white-text">Google 계정으로 로그인해 주세요.</small>
                                                        </>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {isDriveInitialized && (
                                                <>
                                                    ...
                                                    <div className="glass-card mb-4">
                                                        ...
                                                        {!spreadsheetId && (
                                                            <div className="mt-3 small" style={{ opacity: .85 }}>
                                                                <i className="fas fa-info-circle me-2"></i>
                                                                시트 파일이 없습니다. 먼저 생성해 주세요.
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                                        <div className="d-flex align-items-center gap-2">
                                                            {currentPath.length > 0 && (
                                                                ...
                                                                >
                                                                    <i className="fas fa-arrow-left"></i> 뒤로
                                                                </button>
                                                            )}
                                                            <h4 className="mb-0">
                                                        ...
                                                        <div className="btn-group" role="group">
                                                            ...
                                                            >
                                                                <i className="fas fa-folder"></i> 포트폴리오
                                                            </button>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="file-list">
                                                        {isDriveLoading ? (
                                                            ...
                                                            <div className="empty-state py-5">
                                                                ...
                                                                <small className="white-text">오른쪽 상단의 업로드 버튼으로 파일을 추가하세요.</small>
                                                            </div>
                                                        ) : (
                                                            <>
                                                                {/* 폴더들 */}
                                                                {driveFiles
                                                                    ...
                                                                    .map((file, index, array) => (
                                                                        <div key={file.id} className="list-group-item file-item folder-item glass-row" onClick={() => enterFolder(file.id, file.name)}>
                                                                            <div className="d-flex align-items-center">
                                                                                <i className="fas fa-folder me-3 folder-icon"></i>
                                                                                <div className="flex-grow-1">
                                                                                    <h6 className="mb-1 folder-name">{file.name}</h6>
                                                                                    <small className="white-text">폴더</small>
                                                                                </div>
                                                                                <div className="file-size">
                                                                                    <small className="white-text">-</small>
                                                                                </div>
                                                                                <div className="file-modification-date">
                                                                                    <small className="white-text">수정 {new Date(file.modifiedTime).toLocaleDateString()}</small>
                                                                                </div>
                                                                ...
                                                                {/* 폴더/파일 구분선 */}
                                                                {driveFiles.filter(f => f.mimeType === 'application/vnd.google-apps.folder').length > 0 &&
                                                                    driveFiles.filter(f => f.mimeType !== 'application/vnd.google-apps.folder').length > 0 && (
                                                                ...
                                                                {driveFiles
                                                                    .filter(file => file.mimeType !== 'application/vnd.google-apps.folder')
                                                                    .map((file, index, array) => (
                                                                        <div key={file.id} className="list-group-item file-item glass-row">
                                                                            <div className="d-flex align-items-center">
                                                                                <i
                                                                                    className={`fas ${
                                                                                        file.mimeType === 'application/vnd.google-apps.spreadsheet' ? 'fa-file-excel' :
                                                                                            file.mimeType === 'application/vnd.google-apps.document' ? 'fa-file-word' :
                                                                                                file.mimeType === 'application/vnd.google-apps.presentation' ? 'fa-file-powerpoint' :
                                                                                                    file.mimeType.startsWith('image/') ? 'fa-file-image' : 'fa-file'
                                                                                    } me-3 ${
                                                                                        file.mimeType === 'application/vnd.google-apps.spreadsheet' ? 'file-type-spreadsheet' :
                                                                                ...
                                                                                ></i>
                                                                                <div className="flex-grow-1">
                                                                                    <h6 className="mb-1 file-name" onClick={() => openFileInNewTab(file)}>
                                                                                        {file.name}
                                                                                    </h6>
                                                                                    <small className="white-text">{getFileTypeDisplay(file)}</small>
                                                                                </div>
                                                                                <div className="file-size">
                                                                                    <small className="white-text">{formatFileSize(file.size)}</small>
                                                                                </div>
                                                                                <div className="file-modification-date">
                                                                                    <small className="white-text">수정 {new Date(file.modifiedTime).toLocaleDateString()}</small>
                                                                                </div>
                    ...
                    <footer className="main-footer">
                        <div className="footer-content">
                            ...
                            <p className="footeropyright">© 2025 Portra. All rights reserved.</p>
                        </div>
                    </footer>
                </div>
            )}
